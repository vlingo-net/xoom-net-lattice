...
namespace Vlingo.Tests.Lattice.Model.Projection
{
    public class JournalProjectionDispatcherTest
    {
        ...
        private IAppendResultInterest appendInterest;
        private IDispatcher<Dispatchable<IEntry, IState>> dispatcher;
        ...
        public void TestThatThreeEventProject()
        {
            ...
            Assert.Equal(1, accessHolder.AccessJournal.ReadFrom<int>(AccessJournal));
            Assert.Equal(1, accessHolder.AccessProjection.ReadFrom<int>(AccessProjection));
        }
        ...
        public JournalProjectionDispatcherTest(ITestOutputHelper output)
        {
            ...
            var descriptions = new List<ProjectToDescription>
            {
                ProjectToDescription<OneHappenedProjectionActor>.With<OneHappened>(() =>
                    new OneHappenedProjectionActor(accessHolder)),
                ProjectToDescription<TwoHappenedProjectionActor>.With<TwoHappened>(() =>
                    new TwoHappenedProjectionActor(accessHolder)),
                ProjectToDescription<AllHappenedProjectionActor>
                    .With<OneHappened>(() => new AllHappenedProjectionActor(accessHolder))
                    .AndWith<TwoHappened>()
            ...
            journal = Journal<string>.Using<InMemoryJournalActor<string>>(world.Stage, dispatcher);

            EntryAdapterProvider.Instance(world).RegisterAdapter(new OneHappenedAdapter());
            EntryAdapterProvider.Instance(world).RegisterAdapter(new TwoHappenedAdapter());
            EntryAdapterProvider.Instance(world).RegisterAdapter(new ThreeHappenedAdapter());

            appendInterest = world.Stage.ActorFor<IAppendResultInterest>(() => new JournalAppendResultInterest());
        }
        ...
        public class AccessHolder
        {
            ...
            public AccessSafely AccessJournalFor(int times)
            {
            ...
            public AccessSafely AccessProjectionFor(int times)
            {
